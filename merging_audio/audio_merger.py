...
def merge_audio_files(
    ...
    for entry in audio_entries:
        if not 0.0 <= entry.Intensity <= 1.0:
            raise ValueError(f"Intensity must be between 0.0 and 1.0, got {entry.Intensity}")
    ...
    for entry in audio_entries:
        ...
        if isinstance(entry.Data, str):
            ...
            raise TypeError(f"Data must be either str (file path) or AudioSegment, got {type(entry.Data)}")
